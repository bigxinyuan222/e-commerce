...
interface ReviewItem {
  id: Id; goodsName: string; userName: string; phone: string; rating: number; content: string
  images: string[]; likes: number; status: ReviewStatus; createTime: string
  reply: AdminReply | null; replies: ReplyItem[]
}
interface SummaryItem { id: Id; goodsName: string; content: string; reviewCount: number; status: SummaryStatus; createTime: string }
...
async function loadSummaries() {
  ...
  try { const data = await requestJson('/api/v1/admin/review/ailist?page=1&size=10', { headers: headers() }); summaries.value = listFrom(data).map(summaryView) }
