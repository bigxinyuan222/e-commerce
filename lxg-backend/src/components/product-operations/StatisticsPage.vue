<!--
  文件：StatisticsPage.vue
  所属模块：商品运营模块 - 数据统计
  功能说明：数据统计页面，聚合展示订单、销售额、用户、商品、评价等核心运营指标。
  关键功能：
    1. 核心指标卡片（订单量、销售额、客单价、商品总数、新增用户、库存预警等）
    2. 近7/30日交易趋势柱状图
    3. 商品销量排行榜TOP10
    4. 门店销售排行榜
    5. 订单状态分布饼图
  数据来源：并行请求订单、退款、用户、商品、评价、库存等多个API接口
...
const range = ref<7 | 30>(7)            // 趋势图时间范围（7天或30天）
const orders = ref<Order[]>([])         // 订单列表
const refunds = ref<any[]>([])          // 退款列表
const users = ref<any[]>([])            // 用户列表
const reviews = ref<any[]>([])          // 评价列表
const products = ref<any[]>([])         // 商品列表
const warningCount = ref(0)             // 库存预警数量
const sourceErrors = ref<string[]>([])  // 各数据源的加载错误信息
let timer: number | undefined            // 自动刷新定时器

/** 构建请求头，携带Bearer Token */
 ...
 * 通过循环分页直到获取所有数据
 * @param path - API路径
 * @param sizeKey - 分页参数名（'size'或'pageSize'）
 * @param extra - 额外查询参数
 * @returns 合并后的全部数据数组
 */
...
/** 执行单个数据源加载任务，捕获错误并记录到sourceErrors */
...
/**
 * 加载所有数据源
 * 并行请求订单、退款、用户、商品、评价、库存等数据
 * 每个数据源独立捕获错误，不影响其他数据源加载
 */
...
<style scoped>
/* 页面整体布局 */
.statistics-page{display:flex;flex-direction:column;gap:16px;min-width:0}

/* 页面标题区样式 */
.statistics-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:16px}.statistics-heading h1{margin:0 0 4px;font-size:24px;color:#1e293b}.statistics-heading p{margin:0;color:#64748b;font-size:14px}.refresh-box{display:flex;align-items:center;gap:10px;color:#94a3b8;font-size:12px}

/* 数据源错误提示样式 */
.source-warning{display:flex;gap:8px;padding:10px 12px;border:1px solid #fde68a;border-radius:8px;background:#fffbeb;color:#92400e;font-size:13px}

/* 指标卡片网格样式 */
.statistics-cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.range-actions{display:flex;gap:8px}

/* 交易趋势柱状图样式 */
.sales-chart{height:220px;display:flex;align-items:flex-end;gap:8px;overflow-x:auto;padding:20px 4px 0}.bar-group{height:190px;min-width:24px;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end}.bar{width:min(30px,75%);min-height:4px;border-radius:5px 5px 0 0;background:linear-gradient(180deg,#667eea,#4f6ef7)}.bar-value{font-size:10px;color:#64748b;white-space:nowrap;margin-bottom:4px}.bar-label{font-size:11px;color:#94a3b8;margin-top:6px}

/* 排行榜布局样式 */
.statistics-layout{display:grid;grid-template-columns:minmax(0,2fr) minmax(280px,1fr);gap:16px;align-items:start}.statistics-side{display:flex;flex-direction:column;gap:16px}

/* 排行榜列表项样式 */
.ranking-list,.status-list{display:flex;flex-direction:column;gap:10px}.ranking-item{display:grid;grid-template-columns:28px 1fr auto;align-items:center;gap:9px}.rank-index{display:grid;place-items:center;width:26px;height:26px;border-radius:6px;background:#4f6ef7;color:#fff;font-size:12px;font-weight:700}.ranking-item div{display:flex;flex-direction:column;min-width:0}.ranking-item strong{font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ranking-item small{color:#94a3b8}.ranking-item b{font-size:13px;color:#4f6ef7}

/* 订单状态分布条样式 */
.status-list>div>div:first-child,.status-list footer{display:flex;justify-content:space-between;font-size:13px}.status-track{height:10px;margin-top:4px;background:#e2e8f0;border-radius:6px;overflow:hidden}.status-track span{display:block;height:100%;border-radius:6px}.status-list footer{padding-top:12px;border-top:1px solid #e2e8f0}.status-list footer b{font-size:18px}

/* 空状态样式 */
.empty{text-align:center;padding:24px;color:#94a3b8}

/* 暗黑模式适配 */
[data-theme='dark'] .statistics-heading h1{color:#e5e7eb}[data-theme='dark'] .statistics-heading p{color:#94a3b8}[data-theme='dark'] .source-warning{background:#422006;border-color:#854d0e;color:#fde68a}[data-theme='dark'] .status-track{background:#334155}

/* 响应式适配 */
@media(max-width:1200px){.statistics-cards{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:900px){.statistics-layout{grid-template-columns:1fr}}@media(max-width:600px){.statistics-heading{align-items:flex-start;flex-direction:column}.statistics-cards{grid-template-columns:1fr}.refresh-box{width:100%;justify-content:space-between}}
</style>
