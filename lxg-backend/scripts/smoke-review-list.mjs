...
try {
  ...
  const approvedSummaryRow = page.getByRole('row').filter({ hasText: 'AI摘要联调商品' })
  await approvedSummaryRow.getByRole('button', { name: '通过' }).click()
  await Promise.all([
    page.waitForRequest(request => new URL(request.url()).pathname === '/api/v1/admin/edit/review'),
    (async () => {
      await approvedSummaryRow.getByRole('button', { name: '编辑' }).click()
      await page.locator('.summary-modal textarea').fill('编辑后的 AI 摘要内容')
      await page.locator('.summary-modal').getByRole('button', { name: '保存' }).click()
    })(),
  ])
  ...
  await Promise.all([
    ...
    page.getByRole('row').filter({ hasText: '待删除摘要商品' }).getByRole('button', { name: '删除' }).click(),
  ])
  ...
  await Promise.all([
    ...
    page.locator('.user-reply button.danger').click(),
  ])
  ...
  await page.locator('.review-modal').getByRole('button', { name: '通过' }).click()
  ...
  await page.locator('.review-modal textarea').fill('管理员接口联调回复')
  await Promise.all([
  ...
  await page.locator('.review-modal').getByRole('button', { name: '关闭' }).click()
  await Promise.all([
    page.waitForRequest(request => new URL(request.url()).pathname === '/api/v1/admin/review/hide'),
    page.getByRole('row').filter({ hasText: '评价接口联调商品' }).getByRole('button', { name: '隐藏' }).click(),
  ])
