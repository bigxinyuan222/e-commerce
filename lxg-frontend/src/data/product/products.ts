...
// 根据推荐类型获取商品
export function getProductsByRecommendType(type: string): ProductData[] {
  switch (type) {
    case 'new':
      return products.filter(p => p.tags.includes('新品') || p.sales < 5000).slice(0, 6);
    case 'special':
      return products.filter(p => p.originalPrice > p.price && (p.originalPrice - p.price) / p.originalPrice > 0.15).slice(0, 6);
    case 'digital':
      return products.filter(p => p.categoryId === '1').slice(0, 6);
    case 'fashion':
      return products.filter(p => p.categoryId === '3').slice(0, 6);
    default:
      return products.slice(0, 6);
  }
}
